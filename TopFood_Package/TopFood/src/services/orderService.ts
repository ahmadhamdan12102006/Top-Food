...
export const createOrder = async (
  ...
  try {
    ...
    const sanitizedItems = JSON.parse(JSON.stringify(payload.items));
    const sanitizedAddress = deliveryAddress ? JSON.parse(JSON.stringify(deliveryAddress)) : null;
    ...
    const orderData = {
      ...
      customerName: (payload.customerName || 'زبون').trim(),
      ...
      driverNotes: (payload.driverNotes || '').trim(),
    ...
    await runTransaction(db, async (transaction) => {
      if (payload.selectedDriverId && payload.selectedDriverId.trim() !== '') {
        ...
        if (driverSnapshot.exists()) {
          ...
          if (driverData.isActive !== false && !driverData.currentOrderId) {
    ...
    throw new Error(error.message || 'فشل إنشاء الطلب في قاعدة البيانات');
  }
};
