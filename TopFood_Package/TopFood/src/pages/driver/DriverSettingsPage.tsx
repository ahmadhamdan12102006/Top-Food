    ...
    <div className="min-h-screen bg-gray-50 dark:bg-[#0a0a0c] text-gray-900 dark:text-white p-4" dir="rtl">
      <div className="max-w-md mx-auto pt-6">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-amber-500/5 border border-amber-500/10 rounded-2xl p-5 mt-6">
            <h4 className="text-sm font-black text-amber-600 dark:text-amber-400 mb-2">الدعم الفني والمهام</h4>
            <p className="text-xs text-gray-500 mb-4 leading-relaxed">إذا كنت تظهر كـ "في مهمة" ولكن لا يوجد طلبات في قائمتك، اضغط على الزر أدناه لتصفير حالتك.</p>
            <button
              onClick={async () => {
                if (!currentDriverId) return;
                try {
                  ...
                  toast.success('تم تصفير حالتك بنجاح');
                  fetchDriver();
                } catch {
                  toast.error('فشل تصفير الحالة');
                }
              }}
              className="w-full bg-white dark:bg-gray-900 text-amber-600 border border-amber-500/20 font-black py-3 rounded-xl text-xs hover:bg-amber-500/5 transition-all shadow-sm"
            >
              تصفير حالة التوصيل (فك التعليق)
            </button>
          </div>
        </div>
