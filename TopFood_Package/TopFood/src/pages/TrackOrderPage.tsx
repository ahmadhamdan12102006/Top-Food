...
const TrackOrderPage: React.FC = () => {
  ...
  const [driverLocation, setDriverLocation] = useState<{ lat: number; lng: number; updatedAt: number } | null>(null);

  const settings = useSettingsStore((s) => s.settings);
  const fetchSettings = useSettingsStore((s) => s.fetchSettings);
  const storeLocation = settings?.storeLocation || null;
  ...
  // Subscribe to driver live location when order is on_the_way
  useEffect(() => {
    ...
    const unsub = subscribeToDriverLocation(order.assignedDriverId, (loc) => {
      setDriverLocation(loc);
    });

    return () => unsub();
  }, [order?.assignedDriverId, order?.status]);
  ...
  return (
    <div className="container mx-auto max-w-4xl px-4 py-8">
      ...
      {/* Live Map Section */}
      {order.status === 'on_the_way' && storeLocation && order.deliveryAddress?.coords && (
        <div className="mb-6 rounded-3xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-surface-dark sm:p-6">
          <div className="mb-4 flex items-center gap-3">
            ...
            <div className="flex-1">
              <p className="font-black">تتبع السائق مباشرة</p>
              <p className="text-sm text-gray-500">
                {driverLocation ? 'يتم تحديث الموقع تلقائياً' : 'بانتظار موقع السائق...'}
              </p>
            </div>
            {driverLocation && (
              <span className="animate-pulse rounded-full bg-green-100 px-3 py-1 text-xs font-bold text-green-600 dark:bg-green-900/20 dark:text-green-400">
                مباشر
              </span>
            )}
          </div>
          <Suspense fallback={
            <div className="flex items-center justify-center h-[350px] bg-gray-100 dark:bg-gray-900 rounded-2xl">
              <Loader2 className="animate-spin text-gray-400" size={32} />
            </div>
          }>
            <OrderTrackingMap
              storeCoords={storeLocation}
              customerCoords={order.deliveryAddress.coords}
